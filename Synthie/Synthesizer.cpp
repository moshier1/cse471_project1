...
bool CSynthesizer::Generate(double* frame)
{
    ...
    while (m_currentNote < (int)m_notes.size())
    {
        ...
        {
            //m_delayPos = (m_delayPos + 1) % m_quesize;
            break;
        }
        ...
        {
            instrument = &m_flanger;
        }

        else if (note->Instrument() == L"Ringmod")
        {
            instrument = &m_ringmod;
        }
    ...
    {
        double out = 0;
        out = m_chorus.Process(frame[c], m_time, m_delay[c], m_quesize, m_delayPos);
        out = m_flanger.Process(out, m_time, m_delay[c], m_quesize, m_delayPos);
        out = m_ringmod.Process(out, m_time, m_delay[c], m_quesize, m_delayPos);
        frame[c] = out;
    }
    // add output to delay queue
